...
public class LongRidesExercise {
    ...
    public static class AlertFunction extends KeyedProcessFunction<Long, TaxiRide, Long> {
        ...
        public void open(Configuration config) throws Exception {
            rides = getRuntimeContext().getState(new ValueStateDescriptor<>("ride", TaxiRide.class));
        }

        @Override
        public void processElement(TaxiRide ride, Context context, Collector<Long> out) throws IOException {
            TaxiRide firstRide = rides.value();
            if (firstRide != null) {
                if (ride.isStart) {
                    if (Duration.between(ride.eventTime, firstRide.eventTime)
                            .compareTo(Duration.ofHours(2)) > 0) {
                        out.collect(ride.rideId);
                    }
                } else {
                    context.timerService().deleteEventTimeTimer(firstRide.eventTime.plusSeconds(120 * 60).toEpochMilli());
                    if (Duration.between(firstRide.eventTime, ride.eventTime)
                            .compareTo(Duration.ofHours(2)) > 0) {
                        out.collect(ride.rideId);
                    }
                }
        ...
        public void onTimer(long timestamp, OnTimerContext context, Collector<Long> out) throws IOException {
            out.collect(rides.value().rideId);
            rides.clear();
        }
    }
}
