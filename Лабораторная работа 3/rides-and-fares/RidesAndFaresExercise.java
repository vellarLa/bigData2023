...
public class RidesAndFaresExercise {
    ...
    public static class EnrichmentFunction
        ...
        private ValueState<TaxiRide> rideMap;
        private ValueState<TaxiFare> fareMap;
        @Override
        public void open(Configuration config) throws Exception {
            rideMap = getRuntimeContext().getState(new ValueStateDescriptor<>("ride", TaxiRide.class));
            fareMap = getRuntimeContext().getState(new ValueStateDescriptor<>("fare", TaxiFare.class));
        }

        @Override
        public void flatMap1(TaxiRide ride, Collector<RideAndFare> out) throws Exception {
            TaxiFare fare = fareMap.value();
            if (fare != null && fare.rideId == ride.rideId) {
                fareMap.clear();
        ...
        public void flatMap2(TaxiFare fare, Collector<RideAndFare> out) throws Exception {
            TaxiRide ride = rideMap.value();
            if (ride != null && ride.rideId == fare.rideId) {
                rideMap.clear();
                out.collect(new RideAndFare(ride, fare));
            } else {
                fareMap.update(fare);
